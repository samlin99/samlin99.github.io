System.register(['aurelia-framework', 'aurelia-dialog', './map', './weather'], function (_export, _context) {
	...
	var bindable, bindingMode, DialogService, Map, Weather, _desc, _value, _class, _descriptor, _descriptor2, _descriptor3, _class2, _temp, Welcome;
	...
	return {
		...
		execute: function () {
			_export('Welcome', Welcome = (_class = (_temp = _class2 = function () {
				...
				Welcome.prototype.addTodo = function addTodo(e) {
				...
				Welcome.prototype.removeTodo = function removeTodo(i) {
					this.trip.todo = this.removeArray(this.trip.todo, i);
				};

				Welcome.prototype.selectRow = function selectRow(trip, index) {
				...
				Welcome.prototype.save = function save() {
				...
				Welcome.prototype.removeArray = function removeArray(arr, arrIndex) {
					return arr.slice(0, arrIndex).concat(arr.slice(arrIndex + 1));
				};

				Welcome.prototype.deleteTrip = function deleteTrip() {
					this.trips = this.removeArray(this.trips, this.currentIndex);
					this.resetData();
					localStorage.setItem("tripPlannerData", JSON.stringify(this.trips));
				};

				Welcome.prototype.openMap = function openMap(tgtLocation) {
					this.dialogService.open({ viewModel: Map, model: tgtLocation }).then(function (response) {
						if (!response.wasCancelled) {} else {}
					});
				};

				Welcome.prototype.openWeather = function openWeather(tgtLocation) {
				...
				Welcome.prototype.resetData = function resetData() {
					this.currentIndex = null;
					this.trip = { title: '',
						dest: '',
						startDate: '',
